...
const BSPLTable: React.FC<BSPLTableProps> = ({ headers, data, selectedYears, labelColumnCount }) => {
  ...
  const isTotalRow = (text: string | undefined | null) => {
    ...
    return lower.includes("jumlah") || lower.includes("total") || CNTotalKeyword.some((word) => text.includes(word));
  };

  const isProfitRow = (text: string | undefined | null) => {
    if (!text) return false;
    const lower = text.toString().toLowerCase();
    return lower.includes("laba") || lower.includes("profit") || text.includes("利润");
  };
  useEffect(() => {
    if (labelColumnCount === 1 && data.length) {
      const keys = data.map((row) => row[0]).filter((val): val is string => typeof val === "string");
      setOpenKeys(keys);
    }
  ...
  const renderLabelRows = () => {
    ...
    return data.map((row, i) => {
      ...
      const showArrow = isParent && !isTotalRow(labelText?.toString()) && labelColumnCount > 1;

      const paddingLeft = labelCols[labelColumnCount - 1] ? 16 : labelCols[labelColumnCount - 2] ? 24 : labelCols[labelColumnCount - 3] ? 32 : 0;
  ...
  const renderValueRows = () => {
    ...
    return data.map((row, i) => {
      ...
      if (isParent || ((l2 || l3 || labelCols.length > 1) && openKeys.includes(key))) {
        return (
          <View key={`value-${i}`} style={[styles.valueRow, isTotal && styles.totalRow, isProfit && styles.profitRow, { flex: 1 }]}>
